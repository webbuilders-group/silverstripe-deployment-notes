<div id="$Name" class="field<% if $extraClass %> $extraClass<% end_if %>"<% if $ImageUploadEnabled %> data-image-upload-url="$Link('image-upload')"<% end_if %>>
	...
	<span class="description"><%t WebbuildersGroup\\DeploymentNotes\\Forms\\MarkdownField.USES_MARKDOWN '_This field uses <a href="https://guides.github.com/features/mastering-markdown/" target="_blank">markdown</a> for formatting.' %></span>
</div>
